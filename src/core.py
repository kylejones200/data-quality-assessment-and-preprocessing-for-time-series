...
def assess_data_quality(df: pd.DataFrame, value_col: str) -> dict:
    ...
    return {'missing_values': df[value_col].isnull().sum(), 'missing_percentage': df[value_col].isnull().sum() / len(df) * 100, 'duplicates': df.duplicated().sum(), 'outliers': len(df[(df[value_col] > df[value_col].quantile(0.99)) | (df[value_col] < df[value_col].quantile(0.01))]), 'data_range': df[value_col].max() - df[value_col].min(), 'variance': df[value_col].var()}
